<x-filament::page>
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">            
            <div class="bg-yellow-100 rounded-xl shadow p-4 border-[1.5px] border-gray-800 focus:border-gray-800 ring-2 ring-yellow-400">
                <h3 class="text-sm font-medium text-gray-600">Total Transactions with Discount</h3>
                <p class="mt-2 text-2xl font-bold">
                    {{ $discountStats->count() }}
                </p>
            </div>

            <div class="bg-cyan-100 rounded-xl shadow p-4 border-[1.5px] border-gray-800 focus:border-gray-800 ring-2 ring-cyan-500">
                <h3 class="text-sm font-medium text-gray-600">Total Discount Amount</h3>
                <p class="mt-2 text-2xl font-bold text-red-600">
                    Rp {{ number_format($discountStats->sum('total_discount'), 0) }}
                </p>
            </div>
            
            <div class="bg-lime-100 rounded-xl shadow p-4 border-[1.5px] border-gray-800 focus:border-gray-800 ring-2 ring-lime-500">
                <h3 class="text-sm font-medium text-gray-600">Average Discount</h3>
                <p class="mt-2 text-2xl font-bold text-blue-600">
                    Rp {{ number_format($discountStats->avg('total_discount'), 0) }}
                </p>
            </div>
        </div>

        <!-- Discount Details Table -->
        <div class="bg-orange-100 rounded-lg border-[1.5px] border-danger-600 focus:border-danger-600 shadow-lg p-2 md:p-3 flex flex-col w-full ring-2 ring-orange-500">
